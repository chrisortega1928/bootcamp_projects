...
def index():
    ...
    return render_template('home.html', results=date_reults)
...
def view(date):
    ...
    if request.method == 'POST':
        db.execute('inser into food_date (food_id, log_date_id values (?,?)', [request.form['food-select'], date_result
        ['id']])
        db.commit()
    
    d = datetime.strptime(str(date_result['entry_date']),
    '%Y%m%d')
